...
class ProductsService {
    async getAllProducts(req, limit, sort, numberPage, category, stock) {
        try {
            let query = ProductsMongo.getAllProducts()
            ...
            const pages = await ProductsMongo.paginate({ query, numberPage })
    ...
    async findOne(pid) {
        try {
            const productFinder = await ProductsMongo.findOne(pid);
    ...
    async delete(pid) {
        try {
            const productsDeleted = await ProductsMongo.deleteOne(pid);
    ...
    async create(product) {
        try {
            const createdProduct = await ProductsMongo.create(product);
    ...
    async update({ pid, price, stock, status, rest }) {
        try {
            const updatedProduct = await ProductsMongo.update({ pid, price, stock, status, rest })
    ...
    async updateOneProd({ pid, quantity }) {
        try {
            await ProductsMongo.updateOneProd({ pid, quantity });
        } catch (error) {
            console.log(error);
            throw new Error("Unable to get products");
        }
    ...
    async viewsProducts() {
        try {
            const views = await ProductsMongo.viewsProducts()
            return views;
        } catch (error) {
            console.log(error);
            throw new Error("Unable to find the product");
